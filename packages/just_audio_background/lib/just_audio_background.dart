...
final _remotePreviousController = StreamController<void>.broadcast();
final _remoteNextController = StreamController<void>.broadcast();

/// Provides the [init] method to initialise just_audio for background playback.
class JustAudioBackground {
  /// Emits when the platform media notification requests the previous item.
  static Stream<void> get remotePreviousStream =>
      _remotePreviousController.stream;

  /// Emits when the platform media notification requests the next item.
  static Stream<void> get remoteNextStream => _remoteNextController.stream;
...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  Future<void> skipToNext() async {
    if (hasNext) {
      ...
      _remoteNextController.add(null);
    }
  }
  ...
  Future<void> skipToPrevious() async {
    if (hasPrevious) {
      ...
      _remotePreviousController.add(null);
    }
  }
  ...
  void _broadcastState() {
    ...
    final controls = [
      ...
      if (isActivelyPlaying) MediaControl.pause else MediaControl.play,
